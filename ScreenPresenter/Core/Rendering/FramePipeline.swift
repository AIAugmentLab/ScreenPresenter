...
final class MainThreadFrameDispatcher {
    ...
    private func dispatchToMainThread() {
        DispatchQueue.main.async { [weak self] in
            ...
            // 消费最新帧
            guard let pixelBuffer = self.frameSink.consume() else {
                return
            }
...
final class FramePipeline {
    ...
    init(renderMode: FramePipelineRenderMode = .displayLink) {
        self.bufferedSink = BufferedFrameSink()
        self.dispatcher = MainThreadFrameDispatcher(frameSink: bufferedSink)
        self.renderSink = RenderFrameSink(frameSource: bufferedSink)
    ...
    /// 打印诊断信息
    func logDiagnostics(prefix: String = "") {
        let stats = getStats()
        let skipPercent = stats.skipRate * 100
        let modeStr = renderMode == .displayLink ? "DisplayLink" : "MainThread"
        print("\(prefix) 帧统计[\(modeStr)]: 渲染 \(stats.rendered), 跳过 \(stats.skipped) (\(String(format: "%.1f", skipPercent))%)")
    }
}
