 ...
 */
@interface FBManagedProcess <StdInType : id, StdOutType : id, StdErrType : id> : NSObject
...
/**
 ...
 @param exitCode a future that will fire when the process exits. See -[FBManagedProcess exitCode]
 @param signal a future that will fire when the process is signalled. See -[FBManagedProcess signal]
 @param configuration the configuration the process was launched with.
 @param queue the queue to perform actions on.
 @return an implementation of FBManagedProcess.
 */
...
/**
 ...
 @return a Future with the same base behaviour as -[FBManagedProcess exitCode] with additional checking of codes.
 */
