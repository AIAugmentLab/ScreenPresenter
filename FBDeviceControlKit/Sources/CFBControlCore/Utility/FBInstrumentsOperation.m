...
+ (FBFuture<FBInstrumentsOperation *> *)operationWithTargetInternal:(id<FBiOSTarget>)target configuration:(FBInstrumentsConfiguration *)configuration logger:(id<FBControlCoreLogger>)logger
{
  ...
  return [[[[[[[[FBProcessBuilder
    ...
    start]
    onQueue:target.asyncQueue fmap:^ FBFuture * (FBManagedProcess *task) {
    ...
    onQueue:target.asyncQueue map:^ FBInstrumentsOperation * (FBManagedProcess *task) {
...
- (instancetype)initWithTask:(FBManagedProcess *)task traceDir:(NSURL *)traceDir configuration:(FBInstrumentsConfiguration *)configuration queue:(dispatch_queue_t)queue logger:(id<FBControlCoreLogger>)logger
{
  self = [super init];
  if (!self) {
    return nil;
