...
@property (nonatomic, strong, nullable, readwrite) FBManagedProcess<NSNull *, id<FBDataConsumer>, NSNull *> *task;
...
{
  return [[FBFuture
    onQueue:self.queue resolve:^ FBFuture<FBManagedProcess<NSNull *, id<FBDataConsumer>, NSNull *> *> *{
      if (self.task) {
    ...
    onQueue:self.queue map:^(FBManagedProcess<NSNull *, id<FBDataConsumer>, NSNull *> *task) {
      self.task = task;
      return NSNull.null;
    }];
}
...
- (FBFuture<NSNull *> *)stopReading
{
  return [[FBFuture
    onQueue:self.queue resolve:^ FBFuture<NSNumber *> *{
      FBManagedProcess<NSNull *, id<FBDataConsumer>, NSNull *> *task = self.task;
      self.task = nil;
      if (!task) {
        return [[FBControlCoreError
