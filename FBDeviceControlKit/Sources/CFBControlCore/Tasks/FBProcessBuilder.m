...
- (FBFuture<FBManagedProcess *> *)start
{
  return [FBManagedProcess launchProcessWithConfiguration:self.buildConfiguration logger:self.logger];
}

- (FBFuture<FBManagedProcess *> *)runUntilCompletionWithAcceptableExitCodes:(NSSet<NSNumber *> *)exitCodes
{
  dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0);
  return [[self
    start]
    onQueue:queue fmap:^(FBManagedProcess *process) {
